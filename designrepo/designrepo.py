...
def index() -> rx.Component:
    return rx.cond(
                ...
                rx.cond(
                    ...
                    rx.vstack(
                        rx.flex(
                            ...
                            rx.flex(
                                rx.avatar(
                                    src=State.user.gravatar_url,
                                    name=State.user.name,
                                    size="2",
                                    radius="full",
                                ),
                                rx.text(State.user.name, size="2", weight="medium"),
                                rx.button(
                                    rx.icon("log-out"),
                                    "Logout",
                    ...
                    rx.center(
                        rx.vstack(
                            rx.heading(
                                "Requirement Management", size="9", weight="bold"
                            ),
                            rx.text(
        ...
        rx.center(
            rx.vstack(
                rx.heading("DesignRepo", size="9", weight="bold"),
                rx.text(
                    "Please log in to access the design repository.",
                    size="4",
                    color_scheme="gray",
                ),
                rx.button(
